"""
Parche: corrige subcategorías con códigos numéricos en anzuelos y triples.
Los códigos de modelo del fabricante se convierten en nombres descriptivos
basados en el tipo de anzuelo que representan.
Luego regenera el Excel.
"""
...
# ─── 2. MAP CODED SUBCATEGORIES TO DESCRIPTIVE NAMES ─────────────────────────
# Based on analysis of the product names:
#   12146 = "Anzuelo BLACKNICKEL CT" → anzuelo estándar pata corta black nickel
#   12147 = "Anzuelo NICKEL CT" → anzuelo estándar pata corta nickel
#   16902 = "Anzuelo CRYSTAL GOLD CT" → anzuelo crystal gold
#   1971N = "Anzuelo NICKEL CT" → anzuelo beak/pico nickel (modelo clásico)
#   260H  = "Anzuelo NICKEL CT" → anzuelo recto nickel
#   4330  = "Anzuelo NICKEL CT" → anzuelo circular/curvo nickel
#   43621 = "Anzuelo PROFISSIONAL CX" → anzuelo profesional
#   57310 = "Anzuelo SS INOX" → anzuelo inoxidable
#   57315SS = "Anzuelo NICKEL CX INOX" → anzuelo inoxidable nickel
#   65732ES = "Anzuelo CX ULTRA STEEL / PERMATIN" → anzuelo ultra steel
#   65735ES = "Anzuelo NICKEL CX C/ FURO" → anzuelo con ojo (furo = ojo)
#   71525 = "Anzuelo TUNA CIRCLE NICKEL" → anzuelo circular atunero
#   Z-TN3X = "GARATEIA Z-TN3X NICKEL" → triple Z-TN3X

# We'll use the model code as part of the subcategory for identification,
# but make it more descriptive
SUBCAT_FIXES = {
    '12146':    'Mod. 12146 (Black Nickel)',
    '12147':    'Mod. 12147 (Nickel)',
    '16902':    'Mod. 16902 (Crystal Gold)',
    '1971N':    'Mod. 1971N (Nickel)',
    '260H':     'Mod. 260H (Nickel)',
    '4330':     'Mod. 4330 (Nickel)',
    '43621':    'Mod. 43621 (Profesional)',
    '57310':    'Mod. 57310 (Inox)',
    '57315SS':  'Mod. 57315SS (Inox Nickel)',
    '65732ES':  'Mod. 65732ES (Ultra Steel)',
    '65735ES':  'Mod. 65735ES (Con Ojo)',
    '71525':    'Mod. 71525 (Circular Atunero)',
    'Z-TN3X':  'Mod. Z-TN3X (Nickel)',
}

changes = 0
for p in data['productos']:
    sub = p.get('subcategoria', '')
    if sub in SUBCAT_FIXES:
        p['subcategoria'] = SUBCAT_FIXES[sub]
        changes += 1

# Also fix nombre_es: translate remaining Portuguese terms
NOMBRE_FIXES = {
    'Anzuelo': 'Anzuelo',  # already correct
    'C/ 50': 'x50',
    'C/ 100': 'x100',
    'C/ 30': 'x30',
    'C/ 25': 'x25',
...
nombre_changes = 0
for p in data['productos']:
    original = p.get('nombre_es', '')
    fixed = original
    for old, new in NOMBRE_FIXES.items():
...
print(f"✅ Subcategorías corregidas: {changes} productos")
print(f"✅ Nombres pulidos: {nombre_changes} productos")
